...
const LeadsList = () => {
  ...
  const fetchSales = async () => {
    try {
      const response = await axiosInstance.get(`${process.env.REACT_APP_API_URL}/api/leads/getLeads/${storedId}`);
  ...
  const handleEditLead = (leadId) => {
    navigate(`/salesdashboard/editlist/${leadId}`);
  };
  ...
  const handleStatusChange = async (leadId, status) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/leads/${leadId}`, {
        status,
      });
