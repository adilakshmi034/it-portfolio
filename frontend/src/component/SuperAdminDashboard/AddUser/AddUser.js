...
const AddUser = ({ superAdminId }) => {
  const [fullName, setFullName] = useState('');
  const [email, setEmail] = useState('');
  const [mobileNumber, setMobileNumber] = useState('');
  const storedId = localStorage.getItem("super_admin_Id");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.post(`${process.env.REACT_APP_API_URL}/api/users/${storedId}`, {
        fullName,
        email,
        mobileNumber
      });
