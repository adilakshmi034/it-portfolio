...
const EditLeads = () => {
  ...
  if (status === "OTHER" && !comment.trim()) {
    ...
    axiosInstance
      .put(`${process.env.REACT_APP_API_URL}/api/leads/edit/${leadId}`, updatedLead)
      .then((response) => {
        ...
        Swal.fire({
          ...
          navigate("/SuperAdminDashboard/userlist"); // Navigate to leads list
        });
      })
      .catch((error) => {
        ...
        Swal.fire({
          title: "Error!",
          text: error?.response?.data|| error?.response?.data?.message ||"There was an error updating the lead.",
          icon: "error",
          showConfirmButton: false,
          timer: 1000,
        });
